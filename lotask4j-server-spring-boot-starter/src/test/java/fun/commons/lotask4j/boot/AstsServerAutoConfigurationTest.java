...
/**
 * {@link AstsServerAutoConfiguration} 装配冒烟: 开关/环境条件/校验器 bean 名契约。
 * 业务类迁入 (commit 3) 后此处追加 controller/restTemplate bean 名断言。
 *
...
class AstsServerAutoConfigurationTest {
    ...
    /** 满足校验器的最小属性集 (flyway/tenant/sensitive 关闭以最小化) */
    ...
    @Test
    void assemblesPropertiesAndCheckerWhenEnabled() {
        runner.run(context -> {
