...
- **控制台双域路由 (2026-09 起, 仿 benefit4j 四入口裁剪)**: `/platform/**` (平台治理, admin API) 与 `/tenant/**` (租户业务, client/worker API) 两棵路由树 + `meta.domain`; 守卫按登录身份弹回所属域首页 (`store/auth.ts` 的 `identity`, 锚点 = 登录后 `GET /api/v1/auth/me` 回显的 tenantId, 0=平台/>0=租户, 整页刷新由守卫 `ensureIdentity()` 补查)。页面按域各建薄页 (`PlatformTasks` 走 admin `/tasks?tenantId=` 收窄 vs `TaskList` 走 client 域), guide 类组件双树复用 (子跳转跟随当前域前缀)。dev-mock 登录按 client_id 签发带 `tenant_id` claim 的 JWT 形状 token (`ADMIN`→平台 / 其余→租户) 并拦 `/auth/me` 解析回显, e2e 用同形 token 预置登录态。**前端注入 Bearer 的公开面收敛为精确 `/api/v1/auth/token`** — 勿再用 `includes('/api/v1/auth/')` 宽匹配 (会漏掉 /auth/me 的 token 注入)。
...
- **JaCoCo is enforced at 100% line coverage** (bundle 级 + 核心包级, `jacoco-check` 绑定 `verify` 阶段; `mvn test` 不触发门禁, 需 `mvn verify`)。排除项: `AstsApplication` (Spring 引导入口 main)。2026-09 收紧: 309→431 用例补齐全部行缺口。
...
| `AuthMeController` | Console frontend (both identities) | `GET /api/v1/auth/me` — echoes `TenantIdentity.currentTenantId` (0=platform, >0=tenant). Exists because framework4j-tenant JWT payload carries **no tenant_id claim** (identity lives in the Redis session side), so the console cannot self-identify from the token alone. Dual-domain reachable: `@RequiresToken("TENANT")` with **no** domain annotation. |
